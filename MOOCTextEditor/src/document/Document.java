...
public abstract class Document {
	 ...
	 protected int countSyllables(String word)
	{
		...
		int numSyllables=0;
		String lcwrd = word.toLowerCase();
		char[] Word = lcwrd.toCharArray();
		...
		for(int i = 0; i<Word.length; i++) {
			...
			if(c == 'a' || c == 'e' || c == 'i' || c == 'o' || c == 'u' || c == 'y' ) {
				 ...
				 if(i == Word.length -1 && Word[i]=='e' && numSyllables>1 )  {
					 numSyllables-=1;
				 }
	...
	public double getFleschScore()
	{
		...
		double p1 = 1.015 * ((double)getNumWords()/(double)getNumSentences());
		double p2 = 84.6 * ((double)getNumSyllables()/(double)getNumWords());
		
		double score = 206.835 -p1 -p2 ;
